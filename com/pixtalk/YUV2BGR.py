...
def AllTurn(d, height, width):
    for fileName1 in tqdm(d, colour='#FFFFFF', ncols=150):
        ...
        with open(fileName1, 'rb') as f1:
            ...
            if flag:
                frame_size = frame_size * 3 // 2  # 一帧BGR图像所含的像素个数
        ...
        if flag == True:
            ...
            img = cv2.cvtColor(img, cv2.COLOR_YUV2BGR_NV12)  # 由于 opencv 不能直接读取 YUV 格式的文件, 所以要转换一下格式
        else:
            img = yyyy_uv.reshape((height, width)).astype('uint8')  # NV12 的存储格式为：YYYY UV 分布在两个平面（其在内存中为 1 维）

        # 新代码，保证NV12在最后面
        if fileName2.endswith(".NV12"):
            cv2.imwrite(fileName2.replace("NV12", "bmp"), img)  # 改变后缀即可实现不同格式图片的保存(jpg/bmp/png...)
        else:
...
if __name__ == '__main__':
    fheight = 640
    fwidth = 360
    filePath1 = '/home/wenxingwen/Downloads/0521_rgb_common/rgb'
